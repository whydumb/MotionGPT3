...
def render_cli() -> None:
    ...
    from motGPT.render.blender import render
    from motGPT.render.video import Video
    ...
    for path in paths:
        ...
        if cfg.RENDER.MODE == "video":
            frames_folder = os.path.join(
                output_dir,
                path.replace(".npy", "_frames").split('\\')[-1].split("/")[-1])
        else:
            frames_folder = os.path.join(
                output_dir,
                path.replace(".npy", ".png").split('\\')[-1].split("/")[-1])

        # print(frames_folder, output_dir, path)
        # exit()
        out = render(
            data,
            frames_folder,
