...
from .mot_module import MoTMod, MoTLayerNorm
...
class MoTGPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None, modality_num=2):
        ...
        # self.c_attn = nn.ModuleList([Conv1D(3 * self.embed_dim, self.embed_dim) for mch in modality_channels])
        # self.c_proj = nn.ModuleList([Conv1D(self.embed_dim, self.embed_dim) for mch in modality_channels])
    ...
    def forward(self,
        ...
        # layernorm_output = [
        #         self.ln_1.fn[i](hidden_state)
        #         for i, hidden_state in enumerate(hidden_states)
        #     ]
        attn_output, pask_key_values = self.forward_attn_503(
            layernorm_output, 
        ...
        hidden_states = self.apply_residual(attn_output, residual)

        residual = hidden_states
        layernorm_output = self.ln_2(hidden_states)
        # layernorm_output = [
        #         self.ln_2.fn[i](hidden_state)
        #         for i, hidden_state in enumerate(hidden_states)
        #     ]
        # feed_forward_hidden_states = self.mlp(hidden_states)
        feed_forward_hidden_states = [self.mlp.fn[i](hid) for i, hid in enumerate(layernorm_output)]
        # residual connection
        hidden_states = self.apply_residual(feed_forward_hidden_states, residual)
