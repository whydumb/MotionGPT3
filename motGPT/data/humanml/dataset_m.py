...
class MotionDataset(data.Dataset):
                ...
                try:
                    ...
                    if flag:
                        motion_dict[name] = {
                            ...
                            # "length": [len(motion) for m in motion]}
    ...
    def __getitem__(self, item):
        ...
        print('in __getitem__', coin2, m_length, self.unit_length)
        if coin2 == "double":
            m_length = [(length // self.unit_length - 1) * self.unit_length for length in m_length]
        elif coin2 == "single":
            m_length = [(m_length // self.unit_length) * self.unit_length for length in m_length]
        idx = random.randint(0, len(motion) - m_length[0])
        motion = motion[idx:idx + m_length]
        
        # Z Normalization
        motion = (motion - self.mean) / self.std

        return None, None, None, motion, m_length, None, None, None, None, None, None, fname
